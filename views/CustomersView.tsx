...
import { Search, User, Phone, ArrowRight, Star, Filter } from 'lucide-react';
...
const CustomersView: React.FC<CustomersViewProps> = ({ bookings, onCustomerSelect }) => {
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterMode, setFilterMode] = useState<'all' | 'repeat' | 'cancelled'>('all');
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-2">
          <h3 className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">
            {filterMode === 'repeat' ? 'Top Clients' : 'Client List'} ({filteredCustomers.length})
          </h3>
        </div>
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${customer.activeCount >= 2 ? 'bg-emerald-900 text-emerald-400' : 'bg-neutral-100 text-neutral-400'}`}>
                  {customer.activeCount >= 2 ? <Star size={20} fill="currentColor" /> : <User size={20} />}
                </div>
                <div>
              ...
              <div className="text-right flex items-center gap-3">
                <div className="space-y-0.5">
                  <p className="text-sm font-bold text-neutral-900">{customer.activeCount} Bookings</p>
                  <p className="text-[10px] font-bold text-neutral-300 uppercase">Last: {new Date(customer.lastDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</p>
                </div>
