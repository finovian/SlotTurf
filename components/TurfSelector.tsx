...
const TurfSelector: React.FC<TurfSelectorProps> = ({
  turfs,
  selectedTurfId,
  onSelect,
  allowAll = true,
  ...
  const selectedTurf =
    Array.isArray(turfs) ? turfs?.find((t) => t.id === selectedTurfId) : undefined;
  const [isOpen, setIsOpen] = React.useState(false);

  console.log('selectedTurf', selectedTurf, selectedTurf)
      ...
      >
        <div className="w-5 h-5 bg-neutral-900 rounded-full flex items-center justify-center text-[10px] text-white font-bold">
          {isAllSelected ? (
            ...
            selectedTurf?.name?.charAt(0).toUpperCase() 
          )}
        </div>
